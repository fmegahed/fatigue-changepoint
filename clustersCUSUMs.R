...
stride.len.C1 <- cbind(subject10_cusum_norm[,2],
                       subject11_cusum_norm[,2],
                       subject5_cusum_norm[,2],
                       subject1_cusum_norm[,2],
                       subject6_cusum_norm[,2]) %>% rowMeans()
stride.height.C1 <- cbind(subject10_cusum_norm[,3],
                          subject11_cusum_norm[,3],
                          subject5_cusum_norm[,3],
                          subject1_cusum_norm[,3],
                          subject6_cusum_norm[,3]) %>% rowMeans()
stride.dur.C1 <- cbind(subject10_cusum_norm[,4],
                       subject11_cusum_norm[,4],
                       subject5_cusum_norm[,4],
                       subject1_cusum_norm[,4],
                       subject6_cusum_norm[,4]) %>% rowMeans()
C1 <- cbind(subject1_cusum_norm[,1],
            stride.len.C1, stride.height.C1, stride.dur.C1)
colnames(C1)[1] <- "percent.time.from.start"


# C2 based on the dendogram
stride.len.C2 <- cbind(subject2_cusum_norm[,2],
                       subject4_cusum_norm[,2]) %>% rowMeans()
stride.height.C2 <- cbind(subject2_cusum_norm[,3],
                          subject4_cusum_norm[,3]) %>% rowMeans()
stride.dur.C2 <- cbind(subject2_cusum_norm[,4],
                       subject4_cusum_norm[,4]) %>% rowMeans()
C2 <- cbind(subject1_cusum_norm[,1],
            stride.len.C2, stride.height.C2, stride.dur.C2)
colnames(C2)[1] <- "percent.time.from.start"


# C3 membership based on the dendogram
stride.len.C3 <- cbind(subject13_cusum_norm[,2],
                       subject9_cusum_norm[,2],
                       subject15_cusum_norm[,2],
                       subject12_cusum_norm[,2],
                       subject7_cusum_norm[,2],
                       subject8_cusum_norm[,2]) %>% rowMeans()
stride.height.C3 <- cbind(subject13_cusum_norm[,3],
                          subject9_cusum_norm[,3],
                          subject15_cusum_norm[,3],
                          subject12_cusum_norm[,3],
                          subject7_cusum_norm[,3],
                          subject8_cusum_norm[,3]) %>% rowMeans()
stride.dur.C3 <- cbind(subject13_cusum_norm[,4],
                       subject9_cusum_norm[,4],
                       subject15_cusum_norm[,4],
                       subject12_cusum_norm[,4],
                       subject7_cusum_norm[,4],
                       subject8_cusum_norm[,4]) %>% rowMeans()
C3 <- cbind(subject1_cusum_norm[,1],
            stride.len.C3, stride.height.C3, stride.dur.C3)
colnames(C3)[1] <- "percent.time.from.start"

# C4
stride.len.C4 <- subject14_cusum_norm[,2]
stride.height.C4 <- subject14_cusum_norm[,3]
stride.dur.C4 <- subject14_cusum_norm[,4]
C4 <- subject14_cusum_norm
...
g1 <- ggplot(meltdf.stride.length,
  ...
  ggtitle("Average of the CUSUM of Scaled Stride Length per Cluster")
...
g2 <- ggplot(meltdf.stride.height,
  ...
  theme(legend.position="none",
        plot.title = element_text(hjust = 0.5)) +
  ggtitle("Average of the CUSUM of Scaled Stride Height per Cluster")
...
g3 <- ggplot(meltdf.stride.dur,
  ...
  theme(legend.position="bottom",
        plot.title = element_text(hjust = 0.5) ) +
  ggtitle("Average of the CUSUM of Stride Duration per Cluster")
...
gridPlot <- arrangeGrob((g1), 
                        ...
                        mylegend, nrow = 4,
                        heights=c(5,5,5,1))
ggsave('dtwClustersCUSUM.png', gridPlot, height = 11, width= 8.5,
       units = "in", dpi=600)
