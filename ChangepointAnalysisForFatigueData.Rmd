...
## The Approach of Capizzi and Masarotto (dbphase1)
...
## Plots with the MF Data and their Changepoints{.tabset .tabset-fade}
...
for (i in 1:15) {
  cumsum_RPE <- data.frame(matrix(ncol=3, nrow=19)) # need to initialize every time
  ...
  df_transformed <- melt(get(paste0("subject",i,"_medianf")),
    ...
    # Calculation of when a subjects fatigue is >= 13
    fatigue_from_RPE_threshold <- which(RPE[,i+1]>=RatingThresh) %>% 
                                  min() # returns window number
    changepoints_subj_thr[[i]] <- fatigue_from_RPE_threshold * 600 # from 10-minute interval number to seconds
    df_RPE<-data.frame(d1=RPE[1],t1=RPE[i+1]) # to make dataframe of subjective ratings for ggplot
      ...
      changepoints_subj_ecp[[i]] <- fatigue_from_ecp_changepoint*600 # converting from 10 minute windows to seconds
      ...
      changepoints_cusum_subj_ecp[[i]] <- fatigue_ecp_CusumRPE*600
...
# Did not save it since the results are redone in the next section section
# The purpose of this section is to generate the plots
```


## Plots with CUSUM of the MF Data and their Changepoints{.tabset .tabset-fade}
In this section, we perform the following tasks:  
