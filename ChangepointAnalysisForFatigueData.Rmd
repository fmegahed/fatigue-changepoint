...
```{r, load_libraries, message=FALSE,cache=TRUE}
...
```{r load_data, warning=FALSE, cache=TRUE}
...
```
Based on the above analyses, we save the generated data into an R.data file.
```{r save_kinematic_data, warning=FALSE, cache=TRUE}
rm(raw_data,raw_data_size,i,j,num_rows,subject_files,subject_heights)
save.image(file = "RGenerated/FeatureGeneration.RData")
```
